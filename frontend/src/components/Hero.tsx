  ...
  return (
    ...
    >
      <div className="absolute inset-0 z-0">
        <img src={bg1} alt="Elegant wedding backdrop" className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-b from-black/50 via-black/30 to-black/50" />
      </div>

      <div className="relative z-10 max-w-4xl mx-auto text-center px-6 py-16">
        <p className="text-xs md:text-sm tracking-[0.3em] uppercase text-white/80 mb-4">Est. 2025</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center">
          <button
            onClick={() => (onStartPlanning ? onStartPlanning() : scrollToId("vows"))}
          ...
          >
            <span className="relative z-10 flex items-center gap-3">WRITE VOWS</span>
            <span className="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-100 transition" />
          </button>

          <button
            onClick={() => scrollToId("inspiration")}
